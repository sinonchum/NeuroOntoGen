...
|---|---|---|
| P0 | 生产级 extraction provider + repair loop | Xiaomi MiMo + DeepSeek + generic OpenAI-compatible relay + Anthropic Messages API providers are wired. `repair-turtle` now builds prompts from real SHACL violations, calls a provider, and revalidates bounded candidates. OpenAI-compatible HTTP/network retries are available via provider env vars; diagnostics preserve provider request IDs and honor `Retry-After` backoff hints; Xiaomi real smoke still needs a valid key because the local key returned `401 invalid_key`. |
...
- [x] `pytest` 全部通过；
- [x] 最小 `ABoxPayload` 可序列化为 TTL；
- [x] valid TTL 通过 pySHACL；
- [x] invalid TTL 被 pySHACL 拦截；
- [x] self-repair loop 在 mock LLM 下可从失败转为成功；
- [x] repair 超过 `max_retries` 后硬性失败；
- [x] CLI 至少支持 validate；
- [x] Notebook 可执行；
- [x] README 说明如何复现。
...
- [x] Notebook 可作为 reviewer reproduction package。
...
从当前仓库状态看，Phase 1–5 的可复现 MVP 主干已经成型：schema 编译、typed ABox、RDF/Turtle serialization、SHACL validation、structured violation parsing、mock bounded repair、CLI、examples、notebook、benchmark、CI、OWL optional adapter、prompt stability 和 clustering discovery 都已有测试覆盖。

下一步不应继续横向堆新概念，而应把 MVP 从 deterministic/mock demo 推进到可真实使用的 provider-backed extraction/repair slice。
...
2. 继续完善 provider diagnostics：当前已覆盖 retryable HTTP/network retry、status code taxonomy、credential/config errors、request IDs 和标准 `Retry-After` backoff；下一步可补 provider-specific quota headers / remaining-token telemetry；
3. 实现 local-model adapter，保持核心 SDK provider-neutral；
